...
def task_2_aggregation():
    ...
    accounts_with_count = Account.objects.annotate(amount_of_transactions=Count('transactions'))
    
    # 3. Середній баланс по типах
    avg_by_type = Account.objects.values("account_type").annotate(avg=Avg('balance')).values("account_type", 'avg')
...
def task_3_complex():
    ...
    top_customers = Customer.objects.annotate(total_balance = Sum('accounts__balance')).order_by('-total_balance')[:3]    
